  ...
  return (
    <div className='container'>
      <div className="card">
        ...
        <img src={`http://localhost:5000${imovel.imageUrl}`} alt="Imagem do Imóvel" />
...
function PageImoveis() {
  ...
  useEffect(() => {
    async function fetchImoveis() {
      try {
        const response = await api.get('/buscaimoveis');
        if (response.status === 200) {
          const data = await response.data; // Utilizando response.data
          setImoveis(data);
