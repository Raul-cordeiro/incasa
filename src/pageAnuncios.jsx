...
function AnuncioPage() {
  ...
  useEffect(() => {
    async function fetchAnuncioData() {
      try {
        const response = await api.get( '/buscaranuncio');
        ...
        <div className="anuncio-data">
          ...
          {anuncioData.map((anuncio, index) => (
            <div className="anuncio-card" key={index}>
              ...
              {anuncio.imageAnuncio && <img src={'http://localhost:5000' + anuncio.imageAnuncio} alt={`Imagem ${index + 1}`} className="anuncio-image" />}
